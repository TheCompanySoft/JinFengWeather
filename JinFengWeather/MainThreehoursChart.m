...
@implementation MainThreehoursChart
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    for (int i=0; i<[self numberOfitemsInSection:indexPath.section]/8; i++) {
        for (int j =0; j<8; j++) {
            if (j<8) {
                NSString * str = [NSString stringWithFormat:@"%d:00",j*3];
    ...
    NSString *str1=self.windArray[indexPath.row*12];
