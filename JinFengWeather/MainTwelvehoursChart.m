...
@interface MainTwelvehoursChart ()
{
...
@implementation MainTwelvehoursChart
...
-(void)setYLabels
{
    ...
    for (int i=0; i<7; i++) {
        //y轴label
        UILabel * label = [[UILabel alloc] initWithFrame:CGRectMake(5.0,chartCavanHeight-i*levelHeight, 18*[UIUtils getWindowWidth]/320, 10*[UIUtils getWindowHeight]/568)];
        ...
        label.backgroundColor=[UIColor clearColor];
            ...
            label.text = [NSString stringWithFormat:@"%d",(int)(level * i)];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    [[NSNotificationCenter defaultCenter]postNotificationName:@"1" object:indexstr];
     [[NSNotificationCenter defaultCenter]postNotificationName:@"hour12" object:indexstr];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    // 12小时
    for (int i=0; i<[self numberOfitemsInSection:indexPath.section]/2; i++) {
        for (int j =0;j<2; j++) {
            if (j<2) {
                NSString * str = [NSString stringWithFormat:@"%d:00",j*12];
            [xTitles addObject:str];
            }
        }
    }
    NSString *str1=self.windArray[indexPath.row*48];
